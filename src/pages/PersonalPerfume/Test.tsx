...
export default function Test() {
  ...
  const renderStep = () => {
    if (isLoading) {
      return (
        <div className="text-center w-full">
          ...
          <p className="text-[#AB3130]/80">어울리는 향수를 찾는 중이에요!</p>
        </div>
      );
    }
    ...
    if (showResults) {
      ...
      return (
        <div className="text-center w-full">
          <h2 className="text-3xl font-headline font-bold text-[#AB3130] mb-4">추천 향수</h2>
        ...
        return (
          <div className="text-left w-full">
            ...
            <p className="text-[#AB3130]/80 mb-10">
              아래 버튼을 눌러 테스트를 시작하세요.
            </p>
            <button
              onClick={handleNext}
              className="w-full max-w-md text-left px-6 py-3 rounded-full border border-[#AB3130]/50 text-[#AB3130] hover:bg-[#AB3130]/10 transition-colors"
            >
              다음으로
            </button>
          </div>
          ...
          {(step > 0 || showResults || isLoading) && (
            <div className="mb-8">
              ...
              <div className="w-full mb-4">
                <div className="h-2 bg-[#F2D6D6] rounded-full overflow-hidden">
                  <div
                    className="h-2 bg-[#AB3130] transition-all duration-300"
                    style={{ width: `${((showResults || isLoading ? 8 : step) / 8) * 100}%` }}
                  />
                </div>
                <div className="flex justify-between text-xs text-[#AB3130]/70 mt-1 px-1">
                  <span>1</span>
                  <span>2</span>
                  <span>3</span>
                  <span>4</span>
